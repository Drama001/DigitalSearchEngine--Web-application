...
<body>
    ...
    <a href="{{ url( '/search') }}" class="ml-2 mt-2 text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
    < Back
    </a>
    @else
        <div class="navBar">
        ...
        <a href="{{ url( '/') }}" class="ml-2 mt-2 text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
        < Back
        </a>
    @endif
...
<?php
    ...
    //---- To highlight wikiterms ----//
    function highlightWords($searchText, $word){
        $highlighted_string = preg_replace('#'. preg_quote($word) .'#i', '<span style="background-color: rgba(255, 255, 0, 0.5);;">\\0</span>', $searchText);
    ...
    foreach( $searchResult as $source){
        ...
        if($arr1 != null)
        {
          ...
          for( $i = 0; $i < $c; $i++)
          {
              $abstract= highlightWords($abstract,$terms_arr[$i]); 
          } 
        }
        ...
        <div class="mx-2 text-white">
            ...
            <div><span class="font-bold pr-3 capitalize">Author:</span>'.$author.'</div>
                ...
                <a href="/viewPDF/'.$pdf.'" class="font-bold underline flex"> VIEW DOCUMENT 
                <svg class="w-6 h-6 underline" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </a>
        </div>
        ';
